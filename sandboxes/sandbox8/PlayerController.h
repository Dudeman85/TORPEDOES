...
public:
	...
	void Init()
	{
		...
		engine::ecs::AddComponent(winScreen, TextRenderer{ .font = resources::niagaraFont, .text = "", .offset = Vector3(-1.5, 2, 1), .scale = Vector3(0.03f), .color = Vector3(0.5f, 0.8f, 0.2f), .uiElement = true });
		engine::ecs::AddComponent(winScreen, SpriteRenderer{ .texture = resources::uiTextures["winner.png"], .enabled = false, .uiElement = true });
		engine::ecs::AddComponent(winScreen, Transform{ .position = Vector3(0, 0, 0.5f), .scale = Vector3(0.3f) });
	...
	static void OnCollision(Collision collision)
	{
		...
		engine::Transform& playertranform = engine::ecs::GetComponent<engine::Transform>(collision.a);
		engine::PolygonCollider& playerCollider = engine::ecs::GetComponent<engine::PolygonCollider>(collision.a);
		...
		{
			engine::Rigidbody& rigidbody = engine::ecs::GetComponent<engine::Rigidbody>(collision.b);
			engine::Transform& projectransfor = engine::ecs::GetComponent<engine::Transform>(collision.b);
			Projectile& projectile = engine::ecs::GetComponent<Projectile>(collision.b); // Entity is collision.b 
	...
	void Update(GLFWwindow* window, float dt)
	{
		...
		for (auto itr = entities.begin(); itr != entities.end();)
		{
			...
			Transform& transform = engine::ecs::GetComponent<Transform>(entity);
			Transform& modelTransform = engine::ecs::GetComponent<Transform>(player.renderedEntity);
			Rigidbody& rigidbody = engine::ecs::GetComponent<Rigidbody>(entity);
			PolygonCollider& collider = engine::ecs::GetComponent<PolygonCollider>(entity);
	...
	void CreatePlayers(int count, Vector2 startPos, std::vector<ShipType> shipTypes)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			std::vector<Vector2> colliderVerts{ Vector2(2, 2), Vector2(2, -1), Vector2(-5, -1), Vector2(-5, 2) };
			...
			engine::ecs::AddComponent(playerRender, engine::Transform{ .rotation = Vector3(45, 0, 0) });
